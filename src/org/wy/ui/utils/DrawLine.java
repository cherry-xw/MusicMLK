...
public class DrawLine extends View {
	...
	private final float radius = 120;//半径
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < mSpectrumNum; i++) {
			...
			mBytes_draw_point[k++] = origin_x + (float) (Math.cos(angle_real)*(radius-100*mBytes[i]/128));
			mBytes_draw_point[k++] = origin_y + (float) (Math.sin(angle_real)*(radius-100*mBytes[i]/128));
			mPoints[j++] = (float) (Math.cos(angle_real)*radius) + origin_x;
			mPoints[j++] = (float) (Math.sin(angle_real)*radius) + origin_y;
			
			mPoints[j++] = (float) (Math.cos(angle_real)*(radius+100*mBytes[i]/128)) + origin_x;
			mPoints[j++] = (float) (Math.sin(angle_real)*(radius+100*mBytes[i]/128)) + origin_y;
